...
class Validation {
    ...
    validatePassword(password) {
        var passwordStrength = this.determainPasswordStrength(password);
    ...
    formValidation(data){
        ...
        let errorMessages = keys.reduce((accumulator, value) => { return {...accumulator, [value]: ''}}, {});
        let valid = true;
        for(var i = 0; i < keys.length;i++){
            if(keys[i] === 'email') errorMessages.email = this.validateEmail(data.email);
            else if(keys[i] === 'password') {
                var passwordMessage = this.validatePassword(data.password);
