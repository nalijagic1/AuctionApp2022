            ...
            <ul className='category'>
                {categories && categories.map(cat => (
                    <div key={cat.category.id}>
                        <div className={filter ? "listItemfilter" : "listItem"}
                             onClick={() => showSubcategories(cat.category.name)}>
                            <li>{cat.category.name} </li>
                            {filter &&
                            <div>{filter === cat.category.name ?
                                <TiMinus className='icon' style={{fontSize: 16, color: '#252525'}}/> :
                                <TiPlus className='icon' style={{fontSize: 16, color: '#252525'}}/>}</div>
                            }
                        </div>
